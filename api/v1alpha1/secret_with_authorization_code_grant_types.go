...
// SecretWithAuthorizationCodeGrantSpec defines the desired state of a Snowflake Secret
// with OAuth2 authorization code grant flow.
//
...
type SecretWithAuthorizationCodeGrantSpec struct {
	...
	// Timestamp when the OAuth refresh token expires (e.g. '2025-01-06 20:00:00').
	// +kubebuilder:validation:MinLength=1
	// +kubebuilder:validation:MaxLength=255
	OAuthRefreshTokenExpiryTime string `json:"oauthRefreshTokenExpiryTime"`
...
// SecretWithAuthorizationCodeGrantStatus defines the observed state of a Snowflake Secret
// with OAuth2 authorization code grant flow.
...
// +kubebuilder:resource:categories=snowplane,shortName=swacg
...
// SecretWithAuthorizationCodeGrant is the Schema for the Snowflake Secret (OAuth2 Authorization Code Grant) API.
type SecretWithAuthorizationCodeGrant struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   SecretWithAuthorizationCodeGrantSpec   `json:"spec,omitempty"`
	Status SecretWithAuthorizationCodeGrantStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// SecretWithAuthorizationCodeGrantList contains a list of SecretWithAuthorizationCodeGrant.

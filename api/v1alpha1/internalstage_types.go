...
// InternalStageEncryption specifies the encryption type for an internal stage.
type InternalStageEncryption struct {
	// Type is the encryption type for internal stages.
	// +kubebuilder:validation:Enum=SNOWFLAKE_FULL;SNOWFLAKE_SSE
	Type string `json:"type"`
}

// InternalStageDirectoryOptions configures the directory table for an internal stage.
type InternalStageDirectoryOptions struct {
...
// InternalStageSpec defines the desired state of an InternalStage.
//
...
type InternalStageSpec struct {
	...
	// Encryption specifies the encryption settings for the internal stage.
	// +optional
	Encryption *InternalStageEncryption `json:"encryption,omitempty"`

	// Directory configures the directory table settings.
	// +optional
	Directory *InternalStageDirectoryOptions `json:"directory,omitempty" snowflake:"DIRECTORY"`
...
// InternalStageShowOutput mirrors the SHOW STAGES output stored in status.
type InternalStageShowOutput struct {
...
// InternalStageStatus defines the observed state of an InternalStage.
type InternalStageStatus struct {
	...
	ShowOutput *InternalStageShowOutput `json:"showOutput,omitempty"`
...
// InternalStage is the Schema for the internalstages API.
// +kubebuilder:object:root=true
// +kubebuilder:subresource:status
// +kubebuilder:resource:categories=snowplane,shortName=istg
...
type InternalStage struct {
	...
	Spec   InternalStageSpec   `json:"spec,omitempty"`
	Status InternalStageStatus `json:"status,omitempty"`
}

// InternalStageList contains a list of InternalStage.
// +kubebuilder:object:root=true
type InternalStageList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []InternalStage `json:"items"`
}

func init() {
	SchemeBuilder.Register(&InternalStage{}, &InternalStageList{})
}

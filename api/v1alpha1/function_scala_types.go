...
// FunctionScalaSpec defines the desired state of a Snowflake Scala User-Defined Function.
//
...
// Validate checks the FunctionScalaSpec for consistency.
func (s *FunctionScalaSpec) Validate() error {
	var errs []error
	...
	if s.Handler == "" {
		errs = append(errs, fmt.Errorf("spec.handler is required for Scala functions"))
	}

	if s.RuntimeVersion == "" {
		errs = append(errs, fmt.Errorf("spec.runtimeVersion is required for Scala functions"))
	}

	if s.SnowparkPackage == "" {
		errs = append(errs, fmt.Errorf("spec.snowparkPackage is required for Scala functions"))
	}
...
// FunctionScalaStatus defines the observed state of a FunctionScala.
type FunctionScalaStatus struct {
...
// FunctionScala is the Schema for Scala user-defined functions.
// +kubebuilder:object:root=true
// +kubebuilder:subresource:status
// +kubebuilder:resource:categories=snowplane,shortName=fnscala
...
type FunctionScala struct {
	...
	Spec   FunctionScalaSpec   `json:"spec,omitempty"`
	Status FunctionScalaStatus `json:"status,omitempty"`
}

// FunctionScalaList contains a list of FunctionScala.
// +kubebuilder:object:root=true
type FunctionScalaList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []FunctionScala `json:"items"`
}

func init() {
	SchemeBuilder.Register(&FunctionScala{}, &FunctionScalaList{})
}

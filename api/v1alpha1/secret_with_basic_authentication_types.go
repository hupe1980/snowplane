...
// SecretWithBasicAuthenticationSpec defines the desired state of a Snowflake Secret
...
type SecretWithBasicAuthenticationSpec struct {
	...
	// +kubebuilder:validation:MinLength=1
	// +kubebuilder:validation:MaxLength=255
	Password string `json:"password"` //nolint:gosec // G117: not a credential leak
...
// SecretWithBasicAuthenticationStatus defines the observed state of a Snowflake Secret
...
// +kubebuilder:resource:categories=snowplane,shortName=swba
...
// SecretWithBasicAuthentication is the Schema for the Snowflake Secret (Basic Authentication) API.
type SecretWithBasicAuthentication struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   SecretWithBasicAuthenticationSpec   `json:"spec,omitempty"`
	Status SecretWithBasicAuthenticationStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// SecretWithBasicAuthenticationList contains a list of SecretWithBasicAuthentication.
type SecretWithBasicAuthenticationList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []SecretWithBasicAuthentication `json:"items"`
}

func init() {
	SchemeBuilder.Register(&SecretWithBasicAuthentication{}, &SecretWithBasicAuthenticationList{})
}

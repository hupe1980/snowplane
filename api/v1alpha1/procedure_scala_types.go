...
// ProcedureScalaSpec defines the desired state of a Snowflake Scala Stored Procedure.
//
...
type ProcedureScalaSpec struct {
	...
	// Packages lists additional packages.
	// +optional
...
// Validate checks the ProcedureScalaSpec for consistency.
func (s *ProcedureScalaSpec) Validate() error {
	var errs []error
	...
	if s.Handler == "" {
		errs = append(errs, fmt.Errorf("spec.handler is required for Scala procedures"))
	}

	if s.RuntimeVersion == "" {
		errs = append(errs, fmt.Errorf("spec.runtimeVersion is required for Scala procedures"))
	}

	if s.SnowparkPackage == "" {
		errs = append(errs, fmt.Errorf("spec.snowparkPackage is required for Scala procedures"))
	}
...
// ProcedureScalaStatus defines the observed state of a ProcedureScala.
type ProcedureScalaStatus struct {
...
// ProcedureScala is the Schema for Scala stored procedures.
// +kubebuilder:object:root=true
// +kubebuilder:subresource:status
// +kubebuilder:resource:categories=snowplane,shortName=procscala
...
type ProcedureScala struct {
	...
	Spec   ProcedureScalaSpec   `json:"spec,omitempty"`
	Status ProcedureScalaStatus `json:"status,omitempty"`
}

// ProcedureScalaList contains a list of ProcedureScala.
// +kubebuilder:object:root=true
type ProcedureScalaList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []ProcedureScala `json:"items"`
}

func init() {
	SchemeBuilder.Register(&ProcedureScala{}, &ProcedureScalaList{})
}

...
// FunctionSQLSpec defines the desired state of a Snowflake SQL User-Defined Function.
//
...
// Validate checks the FunctionSQLSpec for consistency.
func (s *FunctionSQLSpec) Validate() error {
	var errs []error
	...
	if s.Body == "" {
		errs = append(errs, fmt.Errorf("spec.body is required for SQL functions"))
	}
...
// FunctionSQLStatus defines the observed state of a FunctionSQL.
...
// FunctionSQL is the Schema for SQL user-defined functions.
// +kubebuilder:object:root=true
// +kubebuilder:subresource:status
// +kubebuilder:resource:categories=snowplane,shortName=fnsql
...
type FunctionSQL struct {
	...
	Spec   FunctionSQLSpec   `json:"spec,omitempty"`
	Status FunctionSQLStatus `json:"status,omitempty"`
}

// FunctionSQLList contains a list of FunctionSQL.
// +kubebuilder:object:root=true
type FunctionSQLList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []FunctionSQL `json:"items"`
}

func init() {
	SchemeBuilder.Register(&FunctionSQL{}, &FunctionSQLList{})
}

...
// +kubebuilder:validation:XValidation:rule="self.from.resource.kind in ['Database','Schema','Warehouse','User','AccountRole','DatabaseRole','AccountRoleGrant','DatabaseRoleGrant','ShareGrant','Table','View','Stage','Task','Stream','Tag','NetworkPolicy','ResourceMonitor','MaskingPolicy','RowAccessPolicy','GrantOwnership','StorageIntegration','SecurityIntegration','FileFormat','Pipe','DynamicTable','PasswordPolicy','NetworkRule']",message="spec.from.resource.kind must be a supported Snowplane resource kind"

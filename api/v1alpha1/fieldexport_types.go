...
// +kubebuilder:validation:XValidation:rule="self.from.resource.kind in ['Alert','Database','Schema','Warehouse','User','AccountRole','DatabaseRole','AccountRoleGrant','DatabaseRoleGrant','ShareGrant','AccountRoleAssignment','DatabaseRoleAssignment','TagAssociation','Table','View','Stage','Task','StreamOnTable','StreamOnView','StreamOnExternalTable','StreamOnDirectoryTable','StreamOnDynamicTable','Tag','NetworkPolicy','ResourceMonitor','MaskingPolicy','RowAccessPolicy','GrantOwnership','StorageIntegration','SecurityIntegration','NotificationIntegration','FileFormat','Pipe','DynamicTable','PasswordPolicy','NetworkRule']",message="spec.from.resource.kind must be a supported Snowplane resource kind"

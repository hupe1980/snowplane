...
func newTestStreamOnDirectoryTable(name, namespace string) *snowplanev1alpha1.StreamOnDirectoryTable {
...
func newTestReconciler(mock *mockService, objs ...runtime.Object) *reconciler.GenericReconciler[*snowplanev1alpha1.StreamOnDirectoryTable, Service, *snowflake.StreamObservation] {
	scheme := testutil.TestScheme()
	cb := fake.NewClientBuilder().WithScheme(scheme).
		WithStatusSubresource(&snowplanev1alpha1.StreamOnDirectoryTable{}, &snowplanev1alpha1.ProviderConfig{})
...
func TestReconcile_StandardSuite(t *testing.T) {
	...
	testutil.ReconcileSuiteConfig{
		...
		NewFixture: func(name, ns string) client.Object {
			return newTestStreamOnDirectoryTable(name, ns)
		},
...
func TestReconcile_Create(t *testing.T) {
	...
	assert.Equal(t, snowflake.StreamSourceStage, capturedOpts.SourceType)
	assert.Equal(t, "MY_STAGE", capturedOpts.SourceName)
	got := &snowplanev1alpha1.StreamOnDirectoryTable{}
...
func TestBuildCreateOptions(t *testing.T) {
	...
	assert.Equal(t, snowflake.StreamSourceStage, opts.SourceType)
	assert.Equal(t, "MY_STAGE", opts.SourceName)

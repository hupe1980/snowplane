...
func (a *adapter) PreReconcile(_ context.Context, _ *snowplanev1alpha1.Database) error {
	return nil
}

func (a *adapter) BuildIdentifier(obj *snowplanev1alpha1.Database) reconciler.Identifier {
...
func (a *adapter) BuildAlterOptions(_ context.Context, obj *snowplanev1alpha1.Database, id reconciler.Identifier, obs *reconciler.Observation) (reconciler.AlterOptions, error) {
	aid, err := reconciler.AssertIdentifier[snowflake.AccountObjectIdentifier](id)
	if err != nil {
		return nil, err
	}

	detail, err := reconciler.AssertDetail[*snowflake.DatabaseObservation](obs)
	if err != nil {
		return nil, err
	}
...
func (a *adapter) ApplyObservation(obj *snowplanev1alpha1.Database, obs *reconciler.Observation) {
	detail, ok := obs.Detail.(*snowflake.DatabaseObservation)
	if !ok {
		return
	}
...
func (a *adapter) DetectDrift(obj *snowplanev1alpha1.Database, obs *reconciler.Observation) *drift.Result {
	detail, ok := obs.Detail.(*snowflake.DatabaseObservation)
	if !ok {
...
func (a *adapter) PostCreate(_ *snowplanev1alpha1.Database)                                    {}
func (a *adapter) PostUpdate(_ *snowplanev1alpha1.Database, _ bool, _ reconciler.AlterOptions) {}

func (a *adapter) SupportsCreateOrAlter() bool { return true }

var _ reconciler.ResourceAdapter[*snowplanev1alpha1.Database, Service] = (*adapter)(nil)

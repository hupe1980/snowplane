...
func (a *adapter) BuildIdentifier(table *snowplanev1alpha1.Table) reconciler.Identifier {
	...
	return snowflake.NewSchemaObjectIdentifier(dbName, schemaName, table.Spec.Name)
}
...
func (a *adapter) BuildAlterOptions(_ context.Context, obj *snowplanev1alpha1.Table, id reconciler.Identifier, obs *reconciler.Observation) (reconciler.AlterOptions, error) {
	sid, err := reconciler.AssertIdentifier[snowflake.SchemaObjectIdentifier](id)
	if err != nil {
		return nil, err
	}

	detail, err := reconciler.AssertDetail[*snowflake.TableObservation](obs)
	if err != nil {
		return nil, err
	}
...
func (a *adapter) ApplyObservation(obj *snowplanev1alpha1.Table, obs *reconciler.Observation) {
	detail, ok := obs.Detail.(*snowflake.TableObservation)
	if !ok {
		return
	}
...
func (a *adapter) DetectDrift(obj *snowplanev1alpha1.Table, obs *reconciler.Observation) *drift.Result {
	detail, ok := obs.Detail.(*snowflake.TableObservation)
	if !ok {
...
var _ reconciler.ResourceAdapter[*snowplanev1alpha1.Table, Service] = (*adapter)(nil)

...
func (a *adapter) BuildAlterOptions(ctx context.Context, obj *snowplanev1alpha1.User, id reconciler.Identifier, obs *reconciler.Observation) (reconciler.AlterOptions, error) {
	aid, err := reconciler.AssertIdentifier[snowflake.AccountObjectIdentifier](id)
	if err != nil {
		return nil, err
	}

	detail, err := reconciler.AssertDetail[*snowflake.UserObservation](obs)
	if err != nil {
		return nil, err
	}
...
func (a *adapter) ApplyObservation(obj *snowplanev1alpha1.User, obs *reconciler.Observation) {
	detail, ok := obs.Detail.(*snowflake.UserObservation)
	if !ok {
		return
	}
...
func (a *adapter) DetectDrift(obj *snowplanev1alpha1.User, obs *reconciler.Observation) *drift.Result {
	detail, ok := obs.Detail.(*snowflake.UserObservation)
	if !ok {
...
var _ reconciler.ResourceAdapter[*snowplanev1alpha1.User, Service] = (*adapter)(nil)

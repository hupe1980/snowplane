...
func newTestSecret(name, namespace string) *snowplanev1alpha1.SecretWithBasicAuthentication {
...
func newTestReconciler(mock *mockService, objs ...runtime.Object) *reconciler.GenericReconciler[*snowplanev1alpha1.SecretWithBasicAuthentication, Service, *snowflake.SecretObservation] {
	scheme := testutil.TestScheme()

	cb := fake.NewClientBuilder().WithScheme(scheme).
		WithStatusSubresource(&snowplanev1alpha1.SecretWithBasicAuthentication{}, &snowplanev1alpha1.ProviderConfig{})
	...
	return &reconciler.GenericReconciler[*snowplanev1alpha1.SecretWithBasicAuthentication, Service, *snowflake.SecretObservation]{
		...
		GVK: snowplanev1alpha1.GroupVersion.WithKind("SecretWithBasicAuthentication"),
	}
}
...
func TestReconcile_Create(t *testing.T) {
	...
	assert.Equal(t, snowflake.SecretTypePassword, capturedOpts.SecretType)
	assert.Equal(t, "myuser", capturedOpts.Username)
	assert.Equal(t, "mypassword", capturedOpts.Password)

	got := &snowplanev1alpha1.SecretWithBasicAuthentication{}
...
func TestBuildCreateOptions(t *testing.T) {
	...
	assert.Equal(t, snowflake.SecretTypePassword, opts.SecretType)
...
func TestApplyObservation(t *testing.T) {
	...
	assert.Equal(t, "PASSWORD", *obj.Status.DescribeOutput.SecretType)
}

...
func newTestSecret(name, namespace string) *snowplanev1alpha1.SecretWithAuthorizationCodeGrant {
...
func successfulObservation() *snowflake.SecretObservation {
	return &snowflake.SecretObservation{
		...
		DescribeOutput: map[string]string{
			...
			"oauth_refresh_token_expiry_time": "2025-12-31 23:59:59",
		},
	}
}

func newTestReconciler(mock *mockService, objs ...runtime.Object) *reconciler.GenericReconciler[*snowplanev1alpha1.SecretWithAuthorizationCodeGrant, Service, *snowflake.SecretObservation] {
	scheme := testutil.TestScheme()

	cb := fake.NewClientBuilder().WithScheme(scheme).
		WithStatusSubresource(&snowplanev1alpha1.SecretWithAuthorizationCodeGrant{}, &snowplanev1alpha1.ProviderConfig{})
	...
	return &reconciler.GenericReconciler[*snowplanev1alpha1.SecretWithAuthorizationCodeGrant, Service, *snowflake.SecretObservation]{
		...
		GVK: snowplanev1alpha1.GroupVersion.WithKind("SecretWithAuthorizationCodeGrant"),
	}
}
...
func TestReconcile_Create(t *testing.T) {
	...
	assert.Equal(t, "refresh-token-abc", capturedOpts.OAuthRefreshToken)
	assert.Equal(t, "2025-12-31 23:59:59", capturedOpts.OAuthRefreshTokenExpiryTime)

	got := &snowplanev1alpha1.SecretWithAuthorizationCodeGrant{}
...
func TestBuildCreateOptions(t *testing.T) {
	...
	assert.Equal(t, "refresh-token-abc", opts.OAuthRefreshToken)
	assert.Equal(t, "2025-12-31 23:59:59", opts.OAuthRefreshTokenExpiryTime)
}
